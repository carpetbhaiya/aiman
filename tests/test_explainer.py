...
def test_explain_passes_command_to_llm_if_not_in_cache(fake_llm):
    # 'fakelinuxcommand12345' is not in the cache, so it should hit the LLM
    fake_llm._responses.append("fakelinuxcommand12345: interactive process viewer")
    result = explain_command("fakelinuxcommand12345", fake_llm)
    assert "fakelinuxcommand12345" in result.lower()
    assert len(fake_llm.calls) == 1
    assert fake_llm.calls[0]["user"] == "fakelinuxcommand12345"


def test_explain_strips_whitespace_from_input_for_llm(fake_llm):
    # 'fakeawk' is not in the cache
    fake_llm._responses.append("ok")
    explain_command("  fakeawk  ", fake_llm)
    assert len(fake_llm.calls) == 1
    assert fake_llm.calls[0]["user"] == "fakeawk"
...
def test_explain_stream_uses_cache_if_present(fake_llm):
    ...
    assert len(chunks) == 1
    assert "**`tar`**: Archiving utility" in chunks[0]
    # The LLM should NOT have been called
    assert len(fake_llm.calls) == 0
